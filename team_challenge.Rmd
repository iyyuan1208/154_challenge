...
svm_cv <- tune.svm(SeriousDlqin2yrs ~ ., 
                          data = completed_data_removed_sample_rfimpute, 
                          gamma = 2^(-1:1), 
                          cost = 2^(2:4), 
                          tunecontrol = tune.control(sampling = "fix"))
svm_fit_sampled <- svm(SeriousDlqin2yrs ~ ., 
                       completed_data_removed_sample_rfimpute,
                       gamma = svm_cv$best.parameters$gamma, 
                       cost = svm_cv$best.parameters$cost)
```

```{r}
svm_fit_sampled_test <- predict(svm_fit_sampled, completed_data_test_rfimpute, decision.values = TRUE)
```

```{r}
dv <- attr(svm_fit_sampled_test, "decision.values")
