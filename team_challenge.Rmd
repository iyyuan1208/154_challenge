...
data_train_imp<- data_train_30[,-1]
imp.train <-missForest(data_train_imp,parallelize="forests")
data_train_imp<- cbind(data_train_30$X,imp.train$ximp)
```
#random forest imputation on data_test set

```{r}
registerDoParallel(cores=4)
data_test_imp<- data_test[,-1]
imp.test <-missForest(data_test_imp[,-1],parallelize="forests")
...
#### Import the imputed testing data 

```{r}
data_test_imp <- read.csv('data_test_imp.csv')
data_train_imp <- read.csv('data_train_imp.csv')
data_train_imp<- data_train_imp[-1]
```
...
```{r}
svm_fit_drop_test <- predict(svm_fit_drop, data_test_imputed[,-c(1,2)], decision.values = TRUE)
```
## ROC AUC
```{r}
data_test_1<-cbind(data_test,svm_fit_drop_test)
plot(roc(data_test_1,SeriousDlqin2yrs,svm_fit_drop_test))
roc(data_test_1,SeriousDlqin2yrs,svm_fit_drop_test)
```
...
```{r}
svm_cv <- tune.svm(SeriousDlqin2yrs ~ ., 
                          data = data_train_30_median[,-1], 
                          gamma = 2^(-1:1), 
                          cost = 2^(2:4), 
                          tunecontrol = tune.control(sampling = "fix"))
svm_fit_median <- svm(SeriousDlqin2yrs ~ ., 
                       data_train_30_median[,-1],
                       gamma = svm_cv$best.parameters$gamma, 
                       cost = svm_cv$best.parameters$cost)
```

```{r}
svm_fit_median_test <- predict(svm_fit_median, data_test_imputed, decision.values = TRUE)
```
## ROC AUC
```{r}
data_test_2<-cbind(data_test,svm_fit_median_test)
plot(roc(data_test_2,SeriousDlqin2yrs,svm_fit_median_test))
roc(data_test_2,SeriousDlqin2yrs,svm_fit_median_test)
```
##### SVM using imputation training data

```{r}
svm_cv <- tune.svm(SeriousDlqin2yrs ~ ., 
                          data = data_train_30_rfimpute_completed, 
                          gamma = 2^(-1:1), 
                          cost = 2^(2:4), 
                          tunecontrol = tune.control(sampling = "fix"))
svm_fit_imputed <- svm(SeriousDlqin2yrs ~ ., 
                       data_train_30_rfimpute_completed,
                       gamma = svm_cv$best.parameters$gamma, 
                       cost = svm_cv$best.parameters$cost)
```

```{r}
svm_fit_imputed_test <- predict(svm_fit_imputed, data_test_imputed, decision.values = TRUE)
```

### ROC AUC
```{r}
data_test_3<-cbind(data_test,svm_fit_imputed_test)
plot(roc(data_test_3,SeriousDlqin2yrs,svm_fit_imputed_test))
roc(data_test_3,SeriousDlqin2yrs,svm_fit_imputed_test)
```
...
```{r}
set.seed(47)
rftune_drop <- tuneRF(data_train_drop[, c(-1,-2)], data_train_drop[,2], stepFactor=1.5, improve=1e-5, ntree=500)
m <- rftune_drop[rftune_drop[,2]==min(rftune_drop[,2]),1]
rf_fit_drop <- randomForest(SeriousDlqin2yrs ~ ., 
                       data_train_drop[,-1], importance= TRUE, mtry=m)
```

```{r}
rf_fit_drop_test <- predict(rf_fit_drop, newdata = data_test_imp[,-1])
```
## ROC AUC
```{r}
...
```{r}
set.seed(47)
rftune_drop <- tuneRF(data_train_30_median[, c(-1,-2)], data_train_30_median[,2], stepFactor=1.5, improve=1e-5, ntree=500)
m <- rftune_drop[rftune_drop[,2]==min(rftune_drop[,2]),1]
rf_fit_median <- randomForest(SeriousDlqin2yrs ~ ., 
                       data_train_30_median[,-1], importance= TRUE, mtry=m)
```

```{r}
rf_fit_median_test <- predict(rf_fit_median, newdata = data_test_imp[,-1])
```
## ROC AUC
```{r}
...
```{r}
set.seed(47)
rftune_drop <- tuneRF(data_train_imp[, c(-1,-2,-3)], data_train_imp[,3], stepFactor=1.5, improve=1e-5, ntree=500)
m <- rftune_drop[rftune_drop[,2]==min(rftune_drop[,2]),1]
rf_fit_imp <- randomForest(SeriousDlqin2yrs ~ ., 
                       data_train_imp[,-1], importance= TRUE, mtry=m)
```

```{r}
rf_fit_imp_test <- predict(rf_fit_imp, newdata = data_test_imp[,-1])
```
## ROC AUC
```{r}
