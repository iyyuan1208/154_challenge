...
```{r}
rf_fit_drop_test <- predict(rf_fit_drop, newdata = data_test_imp)
```
...
```{r}
rf_fit_median_test <- predict(rf_fit_median, newdata = data_test_imp)
```
...
```{r}
set.seed(47)
rftune_drop <- tuneRF(data_train_imp[, c(-1,-2)], data_train_imp[,2], stepFactor=1.5, improve=1e-5, ntree=500)
m <- rftune_drop[rftune_drop[,2]==min(rftune_drop[,2]),1]
...
```{r}
rf_fit_imp_test <- predict(rf_fit_imp, newdata = data_test_imp)
```