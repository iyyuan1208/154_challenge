...
```{r}
#random forest imputation on data_removed_sampled training set 
set.seed(47)
md.pattern(data_train_30[,c(7,12)])
data_train_30_rfimpute <- mice(data_train_30[,-1], meth = 'rf', m = 1)
```
...
```{r}
data_test_imputed <- read.csv('test_imput.csv')
```
...
```{r}
dv <- attr(svm_fit_drop_test, "decision.values")
