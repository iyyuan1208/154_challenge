...
data_removed_sample_rfimpute <- mice(data_removed_sample[,-1], meth = 'rf', m = 1)
```
...
completed_data_removed_sample_rfimpute <- complete(data_removed_sample_rfimpute, 1)
```
...
#### svm
```{r}
svm_fit_sampled <- svm(SeriousDlqin2yrs ~ ., completed_data_removed_sample_rfimpute)
```
