...
```{r}
data_train_30_rfimpute_completed <- complete(data_train_30_rfimpute, 1)
```
...
```{r}
dv <- svm_fit_drop
tp <- c() -> fp #initialize true and false positive vectors
ks <- seq(min(dv), max(dv),length.out=nrow(dv))
for (i in 1:nrow(dv)){
...
```{r}
dv <- attr(svm_fit_imputed_test, "decision.values")
tp <- c() -> fp #initialize true and false positive vectors
ks <- seq(min(dv), max(dv),length.out=nrow(dv))
for (i in 1:nrow(dv)){
k <- ks[i]
tp[i] <- mean(dv[1:50] > k)
fp[i] <- mean(dv[51:100] > k)
}
plot(fp,tp, t='l')
abline(0,1)
```
